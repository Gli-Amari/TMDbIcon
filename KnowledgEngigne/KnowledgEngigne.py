...
class KnwoledgeEngine:
    ...
    def getFactsVoteC(self, row):
        ...
        return 'hasVoteC("{}",hasVoteAv,{}).'.format(row[0], row[8])
    ...
    def createFacts(self):
        ...
        with open('./dataset/normalizedDataset.csv', 'r', encoding='utf-8') as csv_file, open('factsKB.pl', 'w',encoding='utf-8') as prolog_file:
    ...
    def KBInterrogation(self):
        ...
        for _, row in self.df.iterrows():
            assertz = ('ratio(Rating,Title).')
            result.append(list(prolog.query(assertz))[0]['Y'])
